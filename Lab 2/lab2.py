...
def non_maximum_suppression_interpol(d_mag, d_angle, display=True):
    ...
    for i in range(1, d_mag.shape[0] - 1):
        for j in range(1, d_mag.shape[1] - 1):
            ...
            if 0 <= angle < 45:
                q = d_mag[i - 1, j] + np.tan((angle) * np.pi / 180) * (d_mag[i - 1, j - 1] - d_mag[i - 1, j])
                r = d_mag[i + 1, j] + np.tan((angle) * np.pi / 180) * (d_mag[i + 1, j + 1] - d_mag[i + 1, j])
            elif 45 <= angle < 90:
                q = d_mag[i, j - 1] + np.tan((90 - angle) * np.pi / 180) * (d_mag[i - 1, j - 1] - d_mag[i, j - 1])
                r = d_mag[i, j + 1] + np.tan((90 - angle) * np.pi / 180) * (d_mag[i + 1, j + 1] - d_mag[i, j + 1])
            elif 90 <= angle < 135:
                q = d_mag[i, j - 1] + np.tan((angle - 90) * np.pi / 180) * (d_mag[i + 1, j - 1] - d_mag[i, j - 1])
                r = d_mag[i, j + 1] + np.tan((angle - 90) * np.pi / 180) * (d_mag[i - 1, j + 1] - d_mag[i, j + 1])
            else:
                q = d_mag[i - 1, j] + np.tan((180 - angle) * np.pi / 180) * (d_mag[i - 1, j + 1] - d_mag[i - 1, j])
                r = d_mag[i + 1, j] + np.tan((180 - angle) * np.pi / 180) * (d_mag[i + 1, j - 1] - d_mag[i + 1, j])
                
            if d_mag[i, j] > q and d_mag[i, j] > r:
                out[i, j] = d_mag[i, j]
    # END
    if display:
    ...
    for _ in range(n):
        ...
        for i in range(1, weak.shape[0] - 1):
            for j in range(1, weak.shape[1] - 1):
                if weak[i, j] and np.sum(out[i - 1:i + 2, j - 1:j + 2]) > 0:
                    out[i, j] = 1
                    weak[i, j] = 0
                    flag = True
...
def hough_vote_lines(img):
    ...
    distances = np.array([rho_interval * r - d for r in range(R)])
    thetas = np.array([theta_interval * t for t in range(T)])
    
    for i in range(Hi):
        for j in range(Wi):
            if img[i, j]:
                for t in range(T):
                    rho = int(i * np.cos(thetas[t]) + j * np.sin(thetas[t])) + d
                    A[rho, t] += 1
    # END
...
def hough_vote_circles(img, radius = None):
    ...
    #   Remember padding
    A = np.zeros((R_max - R_min + 1, h, w))
    R = np.arange(R_min, R_max + 1)
    X = np.arange(h)
    Y = np.arange(w)
    
    #2. Extracting all edge coordinates
    edge_coordinates = np.argwhere(img)
    ...
    for r_idx, r in enumerate(R):
        ...
        for x, y in edge_coordinates:
            x_min, x_max = max(x - r, 0), min(x + r + 1, h)
            y_min, y_max = max(y - r, 0), min(y + r + 1, w)
            mask_x_min, mask_x_max = r - min(r, x), r + min(r, h - x - 1) + 1
            mask_y_min, mask_y_max = r - min(r, y), r + min(r, w - y - 1) + 1
            A[r_idx, x_min:x_max, y_min:y_max] += mask[mask_x_min:mask_x_max, mask_y_min:mask_y_max]
    # END
...
def hough_vote_circles_grad(img, d_angle, radius = None):
    ...
    for r_idx, r in enumerate(R):
        if r % interval == 0:
            ...
            for x, y in edge_coordinates:
                theta = d_angle[x, y]
                x1 = round(int(x - r * np.cos(theta)) / interval) * interval
                y1 = round(int(y - r * np.sin(theta)) / interval) * interval
                x2 = round(int(x + r * np.cos(theta)) / interval) * interval
                y2 = round(int(y + r * np.sin(theta)) / interval) * interval
