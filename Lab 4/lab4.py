...
def calcOpticalFlowHS(
    ...
    # Update flow field iteratively until convergence
    max_iter = 1000
    while max_iter > 0:
        ...
        u = (
            u_avg
            - (Ix * u_avg + Iy * v_avg + It) / (1 / param_lambda + Ix**2 + Iy**2) * Ix
        )
        v = (
            v_avg
            - (Ix * u_avg + Iy * v_avg + It) / (1 / param_lambda + Ix**2 + Iy**2) * Iy
        )

        if np.linalg.norm((u - u_avg) ** 2 + (v - v_avg) ** 2) < param_delta:
            break
...
class Textonization:
    ...
    def training(self, training_imgs):
        ...
        for img in training_imgs:
            ...
            features.append(feats.reshape(-1, feats.shape[-1]))
        features = np.concatenate(np.array(features, dtype=object), axis=0)
        kmeans = MiniBatchKMeans(n_clusters=self.n_clusters, random_state=0).fit(
            features
        )
        self.clustering_centers = kmeans.cluster_centers_
        # Your code ends here #
    ...
    def testing(self, img):
        ...
        kdtree = KDTree(self.clustering_centers)
        indices = kdtree.query(feats, k=1, return_distance=False)
        textons = np.reshape(indices, (img.shape[0], img.shape[1], 1))
        # Your code ends here #
...
def histogram_per_pixel(textons, window_size):
    ...
    for i in range(h):
        for j in range(w):
            ...
            hist = cv2.calcHist(
                [textons[top:bot, left:right]], [0], None, [200], [0, 200]
            )
